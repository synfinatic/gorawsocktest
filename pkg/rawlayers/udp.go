...
func (u *UDP) LayerType() gopacket.LayerType { return layers.LayerTypeUDP }
...
func (udp *UDP) DecodeFromBytes(data []byte, df gopacket.DecodeFeedback) error {
	...
	udp.SrcPort = layers.UDPPort(binary.BigEndian.Uint16(data[0:2]))
	udp.sPort = data[0:2]
	udp.DstPort = layers.UDPPort(binary.BigEndian.Uint16(data[2:4]))
	udp.dPort = data[2:4]
	udp.Length = binary.BigEndian.Uint16(data[4:6])
	udp.Checksum = binary.BigEndian.Uint16(data[6:8])
	udp.BaseLayer = layers.BaseLayer{Contents: data[:8]}
	switch {
	case udp.Length >= 8:
		hlen := int(udp.Length)
		if hlen > len(data) {
