...
func (t *TCP) LayerType() gopacket.LayerType { return layers.LayerTypeTCP }
...
func (t *TCP) SerializeTo(b gopacket.SerializeBuffer, opts gopacket.SerializeOptions) error {
	...
	if opts.ComputeChecksums {
		...
		bytes[16] = 0
		bytes[17] = 0
		csum, err := t.computeChecksum(b.Bytes(), layers.IPProtocolTCP)
		if err != nil {
			return err
		}
		t.Checksum = csum
...
func (t *TCP) ComputeChecksum() (uint16, error) {
	return t.computeChecksum(append(t.Contents, t.Payload...), layers.IPProtocolTCP)
}
...
func (tcp *TCP) DecodeFromBytes(data []byte, df gopacket.DecodeFeedback) error {
	...
	tcp.SrcPort = layers.TCPPort(binary.BigEndian.Uint16(data[0:2]))
	tcp.sPort = data[0:2]
	tcp.DstPort = layers.TCPPort(binary.BigEndian.Uint16(data[2:4]))
	tcp.dPort = data[2:4]
...
func (t *TCP) TransportFlow() gopacket.Flow {
	return gopacket.NewFlow(layers.EndpointTCPPort, t.sPort, t.dPort)
}
